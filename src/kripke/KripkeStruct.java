...
public class KripkeStruct {
	...
	/**
	 * Parse the file that describes the kripke structure and create a 
	 * directed graph.
	 * @param in String
	...
	public void parseKripkeFile(String in) throws ModelException {
		...
		default:
			msg = msg.concat("Malformed File : should have the form - s1,s2,...; t1 : s1-s2, ... ; s1 : p q, ...;\n");
	...
	private void addKripkeTrans(String in) throws ModelException {
		...
		for(String myString : working) {
			...
			if(myString.length()>0) {
				...
				//System.out.println("ttt State1- " + state1 + " ttt State2- " + state2);
			}
		} //for(...working
	...
	//System.out.println("add transition " + state1 + " to " + state2);
		...
		if((k1 != null) && (k2 != null)) {
			...
			msg = msg.concat("States " + state1 + " and " + state2 + " dont exist and can't be added to transition list\n");
			throw new ModelException(msg);
		}	
		else if((k1 == null)) {
			msg = msg.concat("State " + state1 + " doesnt exist and can't be added to transition list\n");
			throw new ModelException(msg);
			//System.out.println(" state 1 doesnt exist ");
		}		
		else if((k2 == null)) {
			msg = msg.concat("State " + state2 + " doesnt exist and can't be added to transition list\n");
			throw new ModelException(msg);
	...
	private void addPrep(String state1, String prep) throws ModelException {
		...
		if((k1 != null)) {
			...
			msg = msg.concat("State " + state1 + " doesnt exist and can't add predicates\n");
			throw new ModelException(msg);
		}
	 ...
	 */
	public KripkeElement getKs() { return headKE; }
	public void setKs(KripkeElement ke) { this.headKE = ke; }
	...
	public KripkeElement getKe() { return ke; }

	/**
	 * Search for a state in the graph with a given name. If found return it, else
	 * throw an exception.
	 * @param s String name of state
	...
	public KripkeElement getKe(String s) throws ModelException {
		...
		msg = msg.concat("No state " +  s + " is avaliable in Kripke structure\n");
		throw new ModelException(msg);
	}

	/**
	 * Reset the kripke structure to evaluate another formula.
	 * The list of satisfied formulas needs to be cleared and the 
	 * index needs to be reset so the Strongly Connected Components 
