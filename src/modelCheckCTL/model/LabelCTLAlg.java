...
public class LabelCTLAlg {
	 ...
	 * @param ks KripkeStruct
	 * @param startState String a state name in the Kripke Structure
	 * @param formula CTLFormula to test on the Kripke Structure
	 * @throws ModelException
	 */
	public LabelCTLAlg(KripkeStruct ks, String startState, CTLFormula formula) throws ModelException {
		super();
	...
	private String checkForFormula(CTLFormula f, KripkeElement ks) {
		...
		if(result){
			return "\nThe formula: { " + f.toString() + " } **SATIFIES** \nthe kripke structure starting from state " + ks.getLabel() + "\n";
		}
		else {
			return "\nThe formula: { " + f.toString() + " } **FAILS** to satifies \nthe kripke structure starting from state " + ks.getLabel() + "\n";
		}
	} //checkForFormula

	/**
	 * Label the kripke structure with the following formula. 
	 * This is a recursive method.
	 * @param f CTLFormula to test on KripkeStruct
	 */
	...
	/**
	 * Label the Kripke Structure with the prepositional atoms into the satified list.
	 * @param f CTLFormula
	 */
	...
	/**
	 * Check if we should label the state with the preposition. 
	 * If already labeled will return false.
	 * @param f CTLFormula we are looking for.
	 * @param list list of prepositons formulas
	 * @param slist list of satisfied formulas
	 * @return boolean
	 */
	private boolean prepositionLabelCheck(CTLFormula f, ArrayList<String> list, ArrayList<CTLFormula> slist) {
		boolean result = false;		
	 ...
	 * label with not.
	 * @param right CTLFormula right hand side
	 */
	 ...
	 * @param f CTLFormula
	 * @param list lsit of CTLFormulas the state satisfies.
	 * @return boolean
	 */
	 ...
	 * @param left CTLFormula
	 * @param right CTLFormula
	 * @param list list of formulas the state satisfies
	 * @return boolean
	 */
	private boolean andLabelCheck(CTLFormula left, CTLFormula right, ArrayList<CTLFormula> list) {
		boolean rt = false;
		boolean lt = false;
		...
		for(KripkeElement k : wks) {
			...
			if(l != null) {
				for(Edge e : l){
					...
					if(exLabelCheck(right,satList) && !formulaExists(new CTLFormula("EX", right),k.getSatArrayList())) {
	 ...
	 * @param r CTLFormula
	 * @param list list of satisfied formulas for the state.
	 * @return boolean
	 */
	...
	private void labelEU(CTLFormula left, CTLFormula right) {
		...
		for(KripkeElement k : wks ) { 
			ArrayList<CTLFormula> satList  = k.getSatArrayList();
			if(euLabelCheck(right, satList) && !formulaExists(new CTLFormula("EU",left,right), k.getSatArrayList())) {
				...
				if(euLabelCheck(left,k.getSatArrayList())) {
					...
					if(l != null) {
						for(Edge e : l){
							KripkeElement kto = (KripkeElement) e.getTo();
							ArrayList<CTLFormula> satList  = kto.getSatArrayList();
							if(euLabelCheck(new CTLFormula("EU",left,right),satList) && !addedList.contains(k) && !formulaExists(new CTLFormula("EU",left,right), k.getSatArrayList())) {
								addedEU = labelWithEU(k, left, right, addedList);
								break;
							} // doesn't contain AF
						} //kto
	 ...
	 * @param f CTLFormula
	 * @param list list of formulas that satisfy the state
	 * @return boolean
	 */
	...
	/**
	 * Indicate if state should be labeled with EU given its satisfies and the next states.
	 * @param k KripkeElement
	 * @param l CTLFormula
	 * @param r CTLFormula
	 * @param list list of satisfied formulas for the state
	 * @return boolean
	 */
	private boolean labelWithEU(KripkeElement k, CTLFormula l, CTLFormula r, ArrayList<KripkeElement> list) {
	...
	private void labelAF(CTLFormula right) {
		...
		while(addedAF) {
			...
			for(KripkeElement k : wks) {
				List<Edge> l = transList.getAdjacent(k);
				if(l != null) {
					if(!addedList.contains(k) && checkAllForAF(right,l) && !formulaExists(new CTLFormula("AF",right), k.getSatArrayList())) {
						addedAF = labelWithAF(k, right, addedList);	
					}
				}//list not null
			} //klist		
	 ...
	 * @param k KripkeElement next
	 * @param r CTLFormula
	 * @param list list of satisfied formulas for state
	 * @return boolean
	 */
	private boolean labelWithAF(KripkeElement k, CTLFormula r, ArrayList<KripkeElement> list) {
	 ...
	 * @param f CTLFormula
	 * @param list list of satisfied formulas
	 * @return boolean
	 */
	private boolean formulaExists(CTLFormula f, ArrayList<CTLFormula> list) {
		boolean guard = false;
		for(CTLFormula l : list) {
			if(l.toString().equals(f.toString())) { guard = true; }
		}
		return guard;
	} //formulaExists
	...
	/**
	 * Run the tarjan algorithm to create the list of Strongly Connected Components.
	 */
	private void setSCC() {
	...
	/**
	 * Get a list of all the active elements given a certain starting point
	 * in the graph. Only reachable states from the start state.
	...
	public ArrayList<KripkeElement> getListActiveElements() {
		...
		for(ArrayList<Node> lk : scc) {
			for(Node k : lk) {
				//System.out.println("active elements " + ((KripkeElement)k).toString());
	...
	//Getters and Setters
	public KripkeStruct getKs() { return ks; }
	public void setKs(KripkeStruct ks) { this.ks = ks; }
	public KripkeElement getKeStart() { return keStart; }
	public void setKeStart(KripkeElement ke) { 	this.keStart = ke; }
	public void setKeStart(String s) throws ModelException { 
		this.keStart = ks.getKe(s); 
	}
	public CTLFormula getFormula() { return formula; }
	public void setFormula(CTLFormula formula) { this.formula = formula; }
	public AdjacencyList getTransList() { return transList; }
	public void setTransList(AdjacencyList transList) { this.transList = transList; }
	public void setTransList(KripkeStruct k) { this.transList = k.getTransList(); }
	public ArrayList<KripkeElement> getWks() { return wks; }
	public void setWks(ArrayList<KripkeElement> wks) { this.wks = wks; }
